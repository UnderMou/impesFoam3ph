...
List<well> ConstantInjection::readWells(const dictionary& wellsDict)
{
    ...
    forAll(wellNames, i)
    {
        ...
        if (!wellDict.found("rate") || wellDict.found("bhp"))
        {
            FatalIOErrorInFunction(wellDict)
                << "Well " << name << " of the ConstantInjection model is rate"
                << " controlled: give rate and no bhp"
                << exit(FatalIOError);
        }

        wells[i].bhpControl = false;
...
void ConstantInjection::source_pEqn
(
    fvScalarMatrix& pEqn,
...
void ConstantInjection::source_SaEqn
(
...
void ConstantInjection::source_SbEqn
(
