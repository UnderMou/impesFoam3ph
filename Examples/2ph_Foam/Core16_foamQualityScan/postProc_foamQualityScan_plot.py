...
if __name__ == '__main__':
    ...
    DPtime_data = pd.read_csv('reference_results/pressure_drop_experimental.dat', delimiter='\t')
    ...
    model_alg = pd.read_csv('reference_results/model_alg.csv')
    ...
    PD_alg = (np.array(muApp_alg/1000) * ut * L / k)/1e6
    print('\n\n',fg_alg.to_numpy(), PD_alg)
    ...
    plt.scatter(fg_exp, 1000*np.array(muApp_IMPES_steady), marker='x', lw=2, s=100, c='b', label = 'Steady-state ImpesFOAM')
    plt.grid(True)
