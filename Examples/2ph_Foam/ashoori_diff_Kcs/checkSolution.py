...
if __name__ == '__main__':
    ...
    paths = ['Kc_0', 'Kc_0_01', 'Kc_1', 'Kc_200']
    titles = ['$K_c = 0$', '$K_c = 0.01$', '$K_c = 1.0$', '$K_c = 200$']
    ...
    for i in range(len(paths)):
        ...
        #
        xs = t_find * (u/phi) * (fw_plus-fw_minus)/(Sw_plus-Sw_minus)

        #
        ax.flatten()[i].set_title(titles[i])
        ax.flatten()[i].plot(x,nD, lw=2,linestyle='-', c='g',label='$n_D$') 
        ax.flatten()[i].plot(x,Sw, lw=2,linestyle='-', c='b',label='$S_w$')   
        ax.flatten()[i].axvline(xs, lw=2,linestyle='-', c='r')   

        ax.flatten()[i].set_xlabel(r'$x$ [m]')
        ax.flatten()[i].set_ylabel(r'$S_w$ [-] / $n_D$ [-]')
        ax.flatten()[i].grid(True)
        ax.flatten()[i].legend(fontsize=14)



    plt.legend()
    plt.tight_layout()
    plt.savefig('profiles_' + str(t_find) + '.pdf', dpi=300)
    plt.show()