...
if __name__ == '__main__':
    ...
    time_dirs = np.arange(0,150,10).tolist()
    print(time_dirs)

    data_dict = parse_openfoam_case('', variables=['Sb','p'],time_dirs=time_dirs)
    
    v = 0.00024601896551724144

    fig, ax = plt.subplots(1,2,figsize=(10,5))
    Sw_s = []
    eta_s = []
    for i in range(1,len(time_dirs)):
        Sw = data_dict['Sb'].iloc[i]
        x = np.linspace(0,0.05,len(Sw))

        # eta_s.append(x-v*time_dirs[i])
        # ax[0].plot(eta_s[-1],Sw, lw=2, c='b')


    ax[0].plot(Sw_s, lw=2, c='b')    
