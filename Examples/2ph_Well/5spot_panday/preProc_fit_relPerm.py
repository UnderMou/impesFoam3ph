...
plt.tight_layout()
plt.savefig('crudeExpData_panday.png', dpi=300)
plt.show()
plt.close()
...
plt.tight_layout()
plt.savefig('fitting_eff_panday.png', dpi=300)
plt.show()
...
# Plot para krw
ax[0].scatter(dt['sw'], dt['krw'],c='k', label='Panday et al. (1994)')
ax[0].plot(Sw_fit, krw_fit, label=fr'Fitting $k_{{rw}}$ ($n_w=${{ {nw_fit:.2f} }})', color='red')
# ax[0].set_xscale('log')
# ax[0].set_yscale('log')
ax[0].set_xlabel('$S_{w}$')
ax[0].set_ylabel('$k_{rw}$')
ax[0].legend(fontsize=12)
ax[0].grid(True)
ax[0].set_ylim([-0.025,.625])
# ax[0].set_title('Ajuste de $k_{rw}$')

# Plot para kro
ax[1].scatter(dt['sw'], dt['kro'],c='k', label='Panday et al. (1994)')
# Se você tiver kro_fit, plote abaixo. Caso contrário, remova essa linha.
ax[1].plot(Sw_fit, kro_fit, label=fr'Fitting $k_{{ro}}$ ($n_o$={{ {no_fit:.2f} }})', color='red')
# ax[1].set_xscale('log')
# ax[1].set_yscale('log')
ax[1].set_xlabel('$S_{w}$')
ax[1].set_ylabel('$k_{ro}$')
ax[1].legend(fontsize=12)
ax[1].grid(True)
# ax[1].set_title('Ajuste de $k_{ro}$')
...
plt.tight_layout()
plt.savefig('fitting_physical_panday.pdf', dpi=300)
plt.show()
