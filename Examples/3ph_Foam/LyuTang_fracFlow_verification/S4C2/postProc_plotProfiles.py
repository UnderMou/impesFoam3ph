...
if __name__ == '__main__':
    ...
    data_Lyu_Sg = pd.read_csv('reference_results/Sg_analytical_LyuC4.2.csv')
    data_Lyu_So = pd.read_csv('reference_results/So_analytical_LyuC4.2.csv')
    ...
    Sg_OF = np.concatenate(([0.6875],Sg_OF))
    ...
    # ax1.set_title(r'$S_g$ profile at $T_D=0.25$')
    ax1.grid()
    ax1.legend()
    ax1.set_ylim([0,0.75])
    ...
    # ax2.set_title(r'$S_o$ profile at $T_D=0.25$')
    ax2.grid()
    ax2.set_ylim([0,0.85])

    plt.tight_layout()
    plt.savefig('LyuC4.2.pdf', dpi=300)
    plt.show()
