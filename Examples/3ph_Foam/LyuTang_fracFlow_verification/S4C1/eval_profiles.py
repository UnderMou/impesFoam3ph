...
data_Lyu_Sg = pd.read_csv('Sg_analytical_LyuC4.csv')
data_Lyu_So = pd.read_csv('So_analytical_LyuC4.csv')
...
ax1.plot(data_Lyu_Sg.iloc[:,0], data_Lyu_Sg.iloc[:,1], color='g', label='Analytical - (Tang, 2019)')
ax1.set_xlabel(r'$x_D$')
ax1.set_ylabel(r'$S_g$')
ax1.set_title(r'$S_g$ profile at $T_D=0.4$')
ax1.grid()
ax1.legend()
...
ax2.set_title(r'$S_o$ profile at $T_D=0.4$')
ax2.grid()
# ax2.set_ylim([-0.05,0.85])

plt.tight_layout()
plt.savefig('LyuC4.png', dpi=300)
plt.show()
