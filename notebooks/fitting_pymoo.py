...
def process_simulation(params, base_case_directory, experiment_name):
    ...
    for i in range(len(idt)):
        ...
        plt.figure()
        plt.plot(x,SoCT,'b')
        plt.scatter(CTscan_results[i]['x_SoCT'],CTscan_results[i]['So_CT'],c='k')
        plt.plot(x,Sc,'gray')
        error = error_So
...
class CTscan(Problem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        with Manager() as manager:
            ...
            print(list(output))
            print('id_min: ', np.argmin(output), ' | f_obj: ', output[np.argmin(output)])
