...
if __name__ == '__main__':
    ...
    BaseCase_dir = '/home/anderson/OpenFOAM/anderson-9/run/impesFoam3ph/simulator/base_cases/Lyu_CT_exp/'
    new_dir = "../simulator/experiments/" + experiment_name
    ...
    fmcap_samples = np.array([sample[3] for sample in X_ED])
    epcap_samples = np.array([sample[4] for sample in X_ED])
    fmoil_samples = np.array([sample[5] for sample in X_ED])
    delta_samples = np.array([sample[6] for sample in X_ED])
    epoil_samples = np.array([sample[7] for sample in X_ED]) 
    ...
    sns.pairplot(data)
    plt.savefig(new_dir + '/pairplot.pdf', dpi=500)
    plt.show()
    # exit()
