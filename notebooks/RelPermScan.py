...
def run_simulation(base_dir, experiment_name, a_exp=None, b_exp=None, c_exp=None, kra_max=None, krb_max=None, krc_max=None, verbose=True):
    """
    ...
    output = template.render(a_exp=a_exp, b_exp=b_exp, c_exp=c_exp, kra_max=kra_max, krb_max=krb_max, krc_max=krc_max)
...
def process_simulation(params, base_case_directory, experiment_name):
    i, (a_exp, b_exp, c_exp, kra_max, krb_max, krc_max) = params
    experiment_name = experiment_name + f"/sample_{i:03d}"
    run_simulation(base_case_directory, experiment_name, a_exp=a_exp, b_exp=b_exp, c_exp=c_exp, kra_max=kra_max, krb_max=krb_max, krc_max=krc_max, verbose=False)

if __name__ == '__main__':

    # INPUT DIRECTORIES
    experiment_name = 'RelPerm_Scan'
    BaseCase_dir = '/home/anderson/OpenFOAM/anderson-9/run/uqsa/impesFoam3ph/simulator/base_cases/Lyu_CT_exp_ScanRelPerm/'
    new_dir = "../simulator/experiments/" + experiment_name
    ...
    a_exp   *=   np.array([1-perc, 1+perc])
    b_exp   *=   np.array([1-perc, 1+perc])
    c_exp   *=   np.array([1-perc, 1+perc])
    kra_max *=   np.array([1-perc, 1+perc])
    krb_max *=   np.array([1-perc, 1+perc])
    krc_max *=   np.array([1-perc, 1+perc])

    a_exp_samples = np.linspace(a_exp[0], a_exp[1], Nsamples)
    b_exp_samples = np.linspace(b_exp[0], b_exp[1], Nsamples)
    c_exp_samples = np.linspace(c_exp[0], c_exp[1], Nsamples)
    kra_max_samples = np.linspace(kra_max[0], kra_max[1], Nsamples)
    krb_max_samples = np.linspace(krb_max[0], krb_max[1], Nsamples)
    krc_max_samples = np.linspace(krc_max[0], krc_max[1], Nsamples)
    ...
    data = pd.DataFrame({
        ...
        'b_exp' : b_exp_samples,
        'c_exp' : c_exp_samples,
        'kra_max' : kra_max_samples,
        'krb_max' : krb_max_samples,
        'krc_max' : krc_max_samples
    })
    
    sns.pairplot(data, corner=True, kind="hist", plot_kws={'bins':Nsamples})
    plt.savefig(new_dir + '/pairplot.png', dpi=100)
    plt.show()
