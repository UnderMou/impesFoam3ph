...
def process_simulation(params, base_case_directory, experiment_name):
    i, (mu_a, mu_b, mu_c, a_exp, b_exp, c_exp, kra_max, krb_max, krc_max) = params
...
if __name__ == '__main__':
    ...
    BaseCase_dir = '/home/anderson/OpenFOAM/anderson-9/run/uqsa/impesFoam3ph/simulator/base_cases/Lyu_CT_exp_relPermVisco/'
    new_dir = "../simulator/experiments/" + experiment_name
    ...
    dists = {
    'num_vars': 9,
    'names': ['mu_g', 'mu_w', 'mu_o', 'ng', 'nw', 'no', 'krg0', 'krw0', 'kro0'],
    'bounds': [[mu_aR*(1-dpr), mu_aR*(1+dpr)],
               [mu_bR*(1-dpr), mu_bR*(1+dpr)],
               [mu_cR*(1-dpr), mu_cR*(1+dpr)],
               [a_expR*(1-dpr), a_expR*(1+dpr)],
               [b_expR*(1-dpr), b_expR*(1+dpr)],
               [c_expR*(1-dpr), c_expR*(1+dpr)],
               [kra_maxR*(1-dpr), kra_maxR*(1+dpr)],
               [krb_maxR*(1-dpr), krb_maxR*(1+dpr)],
               [krc_maxR*(1-dpr), krc_maxR*(1+dpr)]
              ]
    }
    ...
    a_expR_samples = np.array([sample[3] for sample in X_ED])
    b_expR_samples = np.array([sample[4] for sample in X_ED])
    c_expR_samples = np.array([sample[5] for sample in X_ED])
    kra_maxR_samples = np.array([sample[6] for sample in X_ED]) 
    krb_maxR_samples = np.array([sample[7] for sample in X_ED]) 
    krc_maxR_samples = np.array([sample[8] for sample in X_ED]) 
    ...
    data = pd.DataFrame({
        ...
        'mu_bR': mu_bR_samples,
        'mu_cR': mu_cR_samples,
        'a_expR': a_expR_samples,
        'b_expR': b_expR_samples,
        'c_expR': c_expR_samples,
        'kra_maxR': kra_maxR_samples,
        'krb_maxR': krb_maxR_samples,
        'krc_maxR': krc_maxR_samples
    })
